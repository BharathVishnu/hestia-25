...
const Footer = () => {
    ...
    return (
        <div className=" relative footer-container  w-full  py-10  px-4 md:px-16 flex flex-col justify-between  items-center text-general ">
            <div className="absolute top-3 left-3 opacity-[50%] hover:opacity-[100%]"> 
            ...
            <div className='w-full flex flex-row justify-between pt-5 md:pt-10 pl-8 md:pl-20'>
                <div className='relative flex flex-row  justify-start gap-5 md:gap-10'>
                    <div className='absolute top-0 '>
                        <h1 className='text-[13px] md:text-[18px]  text-white font-semibold '>Quick Links</h1>
                    </div>
                    <div className=' grid grid-rows-7 text-[10px] md:text-[15px] mt-12 text-general  md:space-y-1.5'>
                        {quicklinks1.map((e, ind) => (
                            <Link key={ind} className='justify-self-start text-white opacity-70 hover:opacity-100 transition-all ' to={e.link}>{e.title}</Link>
                        ))}
            ...
            <div className='grid grid-cols-7 text-[6px] md:text-[12px]  text-center mt-12 text-general '>
                {links.map((e, ind) => (
                    <Link key={ind} className='text-center text-white opacity-70 hover:opacity-100 transition-all uppercase' to={e.link}>{e.title}</Link>
                ))}
